...
play.ParseMsg = function (d) {
	...
	if (d.match("bestmove")) {
		...
		if (depth > 13) {
			...
			for (var pvseek = 0; pvseek < e.length; pvseek++) {
				if (e[pvseek] == "pv")
					break;
			}
...
play.getFen = function (e, a) {
	...
	result += board + " - - 0 1";
	//console.log(result);
	if (result.indexOf("k") != -1 && result.indexOf("K") != -1)
		return result;
	return "";
}
