...
function loadConfig() {
	...
	if (!room_id)
		...
		shownomal(), initWebsocket(), setTimeout((function () {
				ws.send("roomid=" + room_id);
			}), 1000);
}
...
onMessage = function (d) {
	...
	if (d.match("player")) {
		...
		if (e[1] == "1")
			waitServerPlay = !1, play.my = 1, play.map = comm.arr2Clone(comm.initMap);
		else if (e[1] == "2")
			waitServerPlay = !0, reverseMode = 1, play.my = -1, play.map = comm.arrReverse(comm.initMap);
		return;
	}
